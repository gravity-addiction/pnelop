...
var PNelOP = (function ($) {
  ...
  bootState = function() {
    ...
    $.each(_navLinks,function(k,v) {
      if (!!pURL && !!tURL
       && (pParsed.search.indexOf(v.link)>-1 || pParsed.hash.indexOf(v.link)>-1)
//       && tParsed.search.indexOf(v.link)==-1
//       && tParsed.hash.indexOf(v.link)==-1
    ...
    // Activating Current
    $.each(_navLinks,function(k,v) {
      if ((tParsed.search.indexOf(v.link)>-1 || tParsed.hash.indexOf(v.link)>-1)
       //&& pParsed.search.indexOf(v.link)==-1 
       //&& pParsed.hash.indexOf(v.link)==-1
...
//      document.styleSheets[ls].disabled = "disabled";
  ...
  bootWidgets = function() {
    ...
    while (_objects_total>_objects_processed && _process<25) { // max object lvl
      ...
      for(var io in tWidgets) { 
        ...
        bootWidget(tWidget, io); // Boot Widget Routine
  ...
  bootWidget = function() {
    ...
    if (arguments.length>1) tIdent = arguments[1];
    ...
    tWidget._ident = tIdent;
            
    if ((typeof _widgetCache[tWidget._ident]) == 'object') {
      if (!!_widgetCache[tWidget._ident]._placeMarker && $(_widgetCache[tWidget._ident]._placeMarker).length && $(tContainer).length)
        $(tContainer).remove();
      if (!!_widgetCache[tWidget._ident]._placeMarker && $(_widgetCache[tWidget._ident]._placeMarker).length)
        tContainer = $(_widgetCache[tWidget._ident]._placeMarker);
    } else {
      _widgetCache[tWidget._ident] = tWidget;
    }
    ...
    if ((typeof tContainer) == 'object' || $(tContainer).length) {
      ...
      if ((typeof tWidget._ident) == 'string' && tWidget._ident != '') $(_tContainer).attr('id',tWidget._ident);
    ...
    tWidget._placeMarker = _tContainer;
    ...
    if ((typeof tWidget.loadTimeout) == 'undefined' || (typeof tWidget.loadTimeout) != 'number')
      tWidget.loadTimeout = _dynamicScriptLoadingTimeout;  
    
    _bootDebug('Load Widget Scripts: '+tWidget._ident);
    _bootDebug(tWidget);
    
    //_widgetCache[tWidget._ident] = tWidget; // Move it to Cache Manage it there
    _loadScripts(_widgetCache[tWidget._ident]); // Load Required Scripts for Widget
    _verifyWidget(_widgetCache[tWidget._ident]); // Verification Routine
  ...
  nukeWidget = function() {
    ...
    // Remove Placemarker
    if (arguments.length>1 && arguments[1] == false && tWidget._placeMarker.prop("tagName").toLowerCase() != 'body') {
      $(tWidget._placeMarker).empty();
    } else if (tWidget._placeMarker.prop("tagName").toLowerCase() != 'body') {
      $(tWidget._placeMarker).remove(); delete tWidget._placeMarker;
    }
    
    // Remove Module
    if (arguments.length>2 && arguments[2] == true) {
      delete _widgetModules[tWidget.jsobject];
  ...
  var _verifyLayoutLoading = function() {
    ...
    if (_checkFailedScripts(tLayout)) {
      ...
      _bootDebug(_loading);
      tLayout._loadLoops += 1;
      var nextTimeout = _dynamicScriptLoadingCycleStart;
      for(var i = 0;i<tLayout._loadLoops;i++) nextTimeout*=2;
  ...
  var _verifyWidget = function() {
    ...
    if (arguments.length>0) { tWidget = arguments[0]; } else { return false; }
    
    if (_checkFailedScripts(tWidget)) {
      alert('Some Widget Script Components Failed to Load');
    } else if (!_checkScripts(tWidget)) {
      // Loop Verify Again
      _bootDebug("Need Widget Scripts Loop: "+tWidget._loadLoops);
      tWidget._loadLoops += 1;
      var nextTimeout = _dynamicScriptLoadingCycleStart;
      for(var i = 0;i<tWidget._loadLoops;i++) { nextTimeout*=2; totalTimeout+=nextTimeout; }
      if (totalTimeout <= tWidget.loadTimeout) setTimeout(function() { _verifyWidget(tWidget); },nextTimeout); else _bootDebug("Failed To Load Widget Scripts For "+tWidget._ident);
    } else {
      tWidget._loaded = 1;
  ...
  var _loadWidget = function() {
    ...
    if ((typeof tWidget) != 'object') return false;
    if ((typeof tWidget._placeMarker) == 'object' && tWidget._placeMarker.length)
      tContainer = tWidget._placeMarker;
    ...
    if ((typeof tWidget._ident) == 'string') tIdent = tWidget._ident;
    ...
    if ((typeof _widgetModules[tWidget.jsobject]) != 'object') {
      _bootError('No Reference for '+tWidget.jsobject+' Found in _widgetModules');
      return;
    }

    if ((typeof _widgetModules[tWidget.jsobject]) == 'object')
      pWidgetFunc = _widgetModules[tWidget.jsobject];
  ...
  var _checkFailedScripts = function() {
    ...
    if (arguments.length>0) { tObject = arguments[0]; } else { return false; }
